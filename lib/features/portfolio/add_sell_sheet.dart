...
class _AddSellSheetState extends ConsumerState<AddSellSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await _applyFifo(qty, portfolioId);
  ...
  Future<void> _applyFifo(double sellQty, String portfolioId) async {
    final allLots = await AssetService.getByPortfolio(portfolioId);
    final lots = allLots
        .where((a) => a.symbol == widget.symbol)
        .toList()
    ...
    for (final lot in lots) {
      ...
      if (lot.quantity <= remaining) {
        ...
        final newQty = lot.quantity - remaining;
        remaining = 0;
        await AssetService.update(lot.copyWith(quantity: newQty));
      }
    }
  }
